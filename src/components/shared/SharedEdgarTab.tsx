...
// ── Cross-ref lookup helper ──────────────────────────────────────────────────
function lookupCrossRefs(
  r: MatchResult,
  crossRefIndex?: Record<string, { source: string; data: string }[]>,
): { source: string; data: string }[] {
  if (!crossRefIndex || !r.inDatabase) return [];
  const form = r.filing.form.trim();
  const date = normalizeDate(r.filing.filingDate);
  // Try exact key first, then with 1-day tolerance
  const exact = crossRefIndex[`${form}|${date}`];
  if (exact) return exact;
  // Also try the local filing type as key (may differ in prefix)
  if (r.matchedLocal) {
    const localKey = `${r.matchedLocal.type.trim()}|${date}`;
    if (crossRefIndex[localKey]) return crossRefIndex[localKey];
    // Try date from local filing too (±1 day tolerance)
    const localDate = normalizeDate(r.matchedLocal.date);
    if (localDate !== date) {
      const altKey1 = `${form}|${localDate}`;
      const altKey2 = `${r.matchedLocal.type.trim()}|${localDate}`;
      if (crossRefIndex[altKey1]) return crossRefIndex[altKey1];
      if (crossRefIndex[altKey2]) return crossRefIndex[altKey2];
    }
  }
  return [];
}
...
const SOURCE_COLORS: Record<string, string> = {
  ...
  catalysts: 'var(--gold)',
  company: 'var(--violet)',
  financials: 'var(--sky, #60a5fa)',
  'quarterly-metrics': 'var(--sky, #60a5fa)',
};
...
const FilingRow: React.FC<{
  ...
  ticker: string;
  crossRefIndex?: Record<string, { source: string; data: string }[]>;
}> = ({ r, typeColors, ticker, crossRefIndex }) => {
  ...
  const refs = lookupCrossRefs(r, crossRefIndex);
  ...
  return (
    <div>
      ...
      {refs.length > 0 && (
        <div style={{ margin: '2px 0 4px 19px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {refs.map((ref, i) => (
            <div key={i} style={{
              display: 'flex', alignItems: 'baseline', gap: 8,
              fontSize: 11, lineHeight: 1.5, padding: '1px 0',
            }}>
              <span style={{
                fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px',
                color: SOURCE_COLORS[ref.source] || 'var(--text3)',
                flexShrink: 0, minWidth: 55,
              }}>
                {ref.source}
              </span>
              <span style={{ color: 'var(--text3)' }}>{ref.data}</span>
            </div>
          ))}
        </div>
      )}
...
const FilingList: React.FC<{
  ...
  ticker: string;
  crossRefIndex?: Record<string, { source: string; data: string }[]>;
}> = ({ results, typeColors, filter, ticker, crossRefIndex }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      {displayed.map((r, i) => (
        <FilingRow key={r.filing.accessionNumber || i} r={r} typeColors={typeColors} ticker={ticker} crossRefIndex={crossRefIndex} />
      ))}
      {hiddenCount > 0 && (
      ...
      {loaded && (
        <>
          ...
          <div style={{
            ...
            <FilingList results={results} typeColors={typeColors} filter={filter} ticker={ticker} crossRefIndex={crossRefIndex} />
          </div>
