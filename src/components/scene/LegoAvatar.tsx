  ...
  return (
    ...
    >
      ...
      <g transform={`scale(${scale})`}>
        {/* Head */}
        <rect x={-9} y={seated ? -56 : -62} width={18} height={18} rx={4}
        ...
        {/* Body */}
        <rect x={-12} y={seated ? -37 : -43} width={24} height={28} rx={3}
          fill={color} opacity={0.9} />
        <text x={0} y={seated ? -19 : -25} textAnchor="middle" fill="#0a0a0a"
          fontSize={8} fontFamily="'Space Mono', monospace" fontWeight={700}>{badge}</text>

        {/* Arms */}
        <g style={{ transformOrigin: `${-12}px ${seated ? -34 : -40}px`,
          transform: `rotate(${actDef.leftArm}deg)`, transition: 'transform 0.5s ease-in-out' }}>
          <rect x={-18} y={seated ? -34 : -40} width={6} height={22} rx={3} fill={color} opacity={0.75} />
        </g>
        <g style={{ transformOrigin: `${12}px ${seated ? -34 : -40}px`,
          transform: `rotate(${actDef.rightArm}deg)`, transition: 'transform 0.5s ease-in-out' }}>
        ...
        {actDef.bubble && (
          <g className="scene-bubble-pop">
            <rect x={-16} y={-84} width={32} height={14} rx={7}
