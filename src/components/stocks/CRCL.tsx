              ...
              <div>
                ...
                <div style={{ background: 'var(--surface2)', borderRadius: 12, padding: 20 }}>
                  ...
                  <div style={{ position: 'relative', height: 40, marginBottom: 24 }}>
                    ...
                    {/* Percentile markers */}
                    {[
                      { pct: 0, label: 'P5', value: mcSim.p5 },
                      { pct: 20, label: 'P25', value: mcSim.p25 },
                      { pct: 50, label: 'P50', value: mcSim.p50 },
                      { pct: 80, label: 'P75', value: mcSim.p75 },
                      { pct: 100, label: 'P95', value: mcSim.p95 },
                    ].map((p, i) => (
                    ...
                    <div style={{
                      position: 'absolute',
                      left: `${Math.min(100, Math.max(0, ((MARKET.price - mcSim.p5) / (mcSim.p95 - mcSim.p5)) * 100))}%`,
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'Space Mono' }}>
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text2)' }}>${mcSim.p5.toFixed(0)}</div>
                      <div style={{ fontSize: 10, color: 'var(--text3)' }}>P5 · {((mcSim.p5 / MARKET.price - 1) * 100).toFixed(0)}%</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 2 }}>${mcSim.p25.toFixed(0)}</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--accent)' }}>${mcSim.p50.toFixed(0)}</div>
                      <div style={{ fontSize: 10, color: 'var(--accent)' }}>Median · {((mcSim.p50 / MARKET.price - 1) * 100).toFixed(0)}%</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 2 }}>${mcSim.p75.toFixed(0)}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text2)' }}>${mcSim.p95.toFixed(0)}</div>
                      <div style={{ fontSize: 10, color: 'var(--text3)' }}>P95 · +{((mcSim.p95 / MARKET.price - 1) * 100).toFixed(0)}%</div>
                    </div>
                  </div>
                </div>
              </div>
