...
export default function BMNRNewsFeed() {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      const res = await fetch("/api/press-intelligence?ticker=BMNR");
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      const raw = (json?.results?.news?.[0]?.newsitem) || [];
      const filtered = raw.filter(isBMNRRelease).sort(
      ...
      <div className="sm-newsfeed-header">
        <div className="sm-newsfeed-top-row">
          <div className="sm-newsfeed-title-group">
            ...
            <span className="sm-newsfeed-ticker">BMNR</span>
          </div>
      ...
      <div className="sm-newsfeed-list">
        ...
        {error && (
          <div className="sm-newsfeed-empty">
            ...
            <div>Check your /api/press-intelligence?ticker=BMNR proxy</div>
          </div>
        )}
        ...
        {!loading && !error && visible.map((item) => {
          ...
          const link = `https://feeds.issuerdirect.com/news-release.html?newsid=${id}&symbol=BMNR`;
          return (
            <div
              key={id}
