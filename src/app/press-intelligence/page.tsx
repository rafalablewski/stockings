...
const FEED_CONFIGS: FeedConfig[] = [
  ...
  },
  {
    ...
    categories: {
      Earnings: (h) => /earnings|results|revenue|q[1-4]\s*20\d\d|financial/i.test(h),
      Satellite: (h) => /satellite|launch|orbit|spectrum|bandwidth|ka-band|constellation|inflight/i.test(h),
      Partnerships: (h) => /partner|agreement|contract|award|select|government|dod|military|defense/i.test(h),
      Corporate: (h) => /acqui|merger|board|director|appoint|officer|ceo|cfo|dividend/i.test(h),
      "Capital Markets": (h) => /notes|offering|convert|shares|capital|repurchase|debt|\$\d/i.test(h),
    },
  },
  {
    ...
    categories: {
      Earnings: (h) => /earnings|results|revenue|q[1-4]\s*20\d\d|financial/i.test(h),
      Launches: (h) => /launch|electron|neutron|mission|payload|orbit|deploy/i.test(h),
      Satellite: (h) => /satellite|spacecraft|photon|component|solar|separation|space system/i.test(h),
  ...
  },
  {
    ...
    categories: {
      Earnings: (h) => /earnings|results|revenue|q[1-4]\s*20\d\d|financial/i.test(h),
      Satellite: (h) => /satellite|launch|orbit|spectrum|hughesnet|jupiter|dish|direct.to.home/i.test(h),
      Connectivity: (h) => /connect|broadband|internet|wireless|5g|network|coverage/i.test(h),
  ...
  },
  {
    ...
    accent: "zinc",
    color: "#A1A1AA",
    colorDim: "rgba(161,161,170,0.15)",
    sourceFilter: () => true,
    headlineFilter: () => true,
    parseResponse: (json) => Array.isArray(json) ? json : json?.results?.news?.[0]?.newsitem || [],
    categories: {
      Earnings: (h) => /earnings|results|revenue|q[1-4]\s*20\d\d|financial/i.test(h),
      "Lunar Missions": (h) => /lunar|moon|lander|odysseus|nova-c|clps|artemis|south pole/i.test(h),
      Partnerships: (h) => /partner|agreement|contract|award|select|nasa|government|dod|darpa/i.test(h),
      Corporate: (h) => /acqui|merger|board|director|appoint|officer|ceo|cfo/i.test(h),
      "Capital Markets": (h) => /notes|offering|convert|shares|capital|repurchase|debt|\$\d/i.test(h),
    },
  },
  {
    ...
    categories: {
      ...
      Mining: (h) => /mining|hash|hashrate|exahash|block|mined|production|megawatt|facility/i.test(h),
      Bitcoin: (h) => /bitcoin|btc|digital asset|crypto|treasury/i.test(h),
      "HPC & AI": (h) => /hpc|high.performance|ai |artificial|gpu|data center|compute|cloud/i.test(h),
      Corporate: (h) => /acqui|merger|board|director|appoint|officer|ceo|cfo/i.test(h),
      "Capital Markets": (h) => /notes|offering|convert|shares|capital|atm|stock|debt|\$\d/i.test(h),
    },
  },
