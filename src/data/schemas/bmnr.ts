...
export const BMNR_SCHEMAS: Record<string, FileSchema> = {
  ...
  'bmnr/purchase-history.ts': {
    file: 'bmnr/purchase-history.ts',
    exportName: 'BMNR_PURCHASE_HISTORY',
    description: 'ETH purchase history with mNAV, price, and cost basis. Newest first.',
    ordering: 'newest first (reverse chronological)',
    entryFields: [
      { name: 'date', type: 'string', required: true, format: 'YYYY-MM-DD', example: "'2026-02-23'" },
      { name: 'ethBought', type: 'number', required: true, format: 'raw ETH count', example: '51162' },
      { name: 'ethPrice', type: 'number', required: true, format: '$/ETH', example: '1958' },
      { name: 'totalEthAfter', type: 'number', required: true, format: 'running total ETH', example: '4422659' },
      { name: 'cashDeployed', type: 'number', required: true, format: 'USD (ethBought × ethPrice)', example: '100175196' },
      { name: 'prevDayClose', type: 'number | null', required: true, format: 'BMNR closing price on last trading day before PR, null if unknown', example: '27.15' },
      { name: 'prevDayMarketCap', type: 'number | null', required: true, format: 'prevDayClose × sharesOutstanding (USD), null if unknown', example: '12_489_000_000' },
      { name: 'navPerShare', type: 'number | null', required: true, format: '(totalEthAfter × ethPrice) / shares, null if unknown', example: '19.97' },
      { name: 'mnavAtTime', type: 'number | null', required: true, format: 'prevDayClose / navPerShare, null if unknown', example: '1.36' },
      { name: 'source', type: 'string', required: true, example: "'Feb 23, 2026 PR'" },
      { name: 'notes', type: 'string', required: false, example: "'3.66% of supply, 73% to Alchemy of 5%.'" },
    ],
    exampleEntry: `  {
    date: '2026-02-23',
    ethBought: 51162,
    ...
    notes: '3.66% of supply, 73% to Alchemy of 5%.',
  },`,
    insertAnchor: '// [PR_CHECKLIST_PURCHASE_HISTORY]',
    notes: [
      'MANDATORY for every BMNR Holdings PR — add new purchase entry at top',
      'ethBought = new ETH holdings - previous ETH holdings (from timeline changes[])',
      'cashDeployed = ethBought × ethPrice (approximate)',
      'mnavAtTime = prevDayClose / navPerShare; set null if stock price not available',
      'navPerShare = (totalEthAfter × ethPrice + cashOnHand) / sharesOutstanding',
      'Also update PURCHASE_HISTORY_METADATA.lastUpdated',
    ],
  },
