...
export const BMNR_SCHEMAS: Record<string, FileSchema> = {
  ...
  'bmnr/purchase-history.ts': {
    ...
    entryFields: [
      ...
      { name: 'mnavAtTime', type: 'number | null', required: true, format: 'stockPrice / navPerShare, null if unknown', example: '1.36' },
      { name: 'stockPrice', type: 'number | null', required: true, format: '$/share, null if unknown', example: '27.15' },
      { name: 'navPerShare', type: 'number | null', required: true, format: '(totalEthAfter × ethPrice + cash) / shares, null if unknown', example: '19.97' },
    ...
    notes: [
      ...
      'mnavAtTime = stockPrice / navPerShare; set null if stock price not available',
