// api/irdm-news.js
// Proxy for IRDM (Iridium Communications) press releases via QuoteMedia/AccessWire
// Deploy at: api/irdm-news.js
// Cache: 5 minutes
let cache = null;
let cacheTime = 0;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // QuoteMedia returns double-encoded JSON — response.json() unwraps outer quotes
    const raw = await upstream.json();
    const json = typeof raw === 'string' ? JSON.parse(raw) : raw;
    const items = json?.results?.news?.[0]?.newsitem ?? [];
    // Keep only official Iridium releases:
    // - source is PR Newswire, Canada Newswire, or Business Wire
    // - headline mentions "iridium" (case-insensitive) to exclude index rebalancing notices
    const officialSources = ['pr newswire', 'canada newswire', 'business wire'];
    ...
    // Deduplicate by permalink (same release on PRN + CNW)
    const seen = new Set();
    ...
    console.error('irdm-news error:', err);
