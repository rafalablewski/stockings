// api/mstr-news.js
// Proxy for MSTR (Strategy, formerly MicroStrategy) press releases via QuoteMedia/AccessWire
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Keep only official Strategy/MicroStrategy releases:
    // - source is a wire service
    // - headline mentions company name or ticker
    ...
    const filtered = items.filter((item) => {
      ...
      return (
        ...
        (/\bstrategy\b/.test(hl) || /microstrategy/i.test(hl) || /\bmstr\b/.test(hl))
      );
    });
    ...
    console.error('mstr-news error:', err);
