// api/gsat-news.js
// Proxy for GSAT (Globalstar Inc.) press releases via QuoteMedia/AccessWire
// Deploy at: api/gsat-news.js
// Cache: 5 minutes
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Keep only official Globalstar releases:
    // - source is Business Wire, PR Newswire, or Canada Newswire
    // - headline mentions "globalstar" (case-insensitive) to exclude noise
    const officialSources = ['business wire', 'pr newswire', 'canada newswire'];
    const filtered = items.filter((item) => {
      const src = (item.source || '').toLowerCase();
      const hl = (item.headline || '').toLowerCase();
      return (
        officialSources.some((s) => src.includes(s)) &&
        hl.includes('globalstar')
      );
    });
    ...
    console.error('gsat-news error:', err);
